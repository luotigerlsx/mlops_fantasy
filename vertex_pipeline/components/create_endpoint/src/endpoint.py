...
from kfp.dsl.io_types import Artifact, Output
...
def get_or_create_endpoint(
    ...
    # If create_if_not_exists is True and no existing endpoint with the display name, create one
    if endpoints:
        model_endpoint = endpoints[0]
        logging.info(f'Endpoint {model_endpoint.name} is found')
    elif create_if_not_exists:
        logging.info(f'Endpoint with display_name {display_name} is not found, create one.')
