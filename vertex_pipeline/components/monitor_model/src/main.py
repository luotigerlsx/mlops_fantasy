...
from google.cloud.aiplatform_v1beta1 import GcsSource, JobServiceClient, ListModelDeploymentMonitoringJobsRequest, \
    ModelDeploymentMonitoringJob, ModelDeploymentMonitoringObjectiveConfig, ModelDeploymentMonitoringScheduleConfig, \
...
JOB_DISPLAY_NAME = 'activism_model_monitoring'

# Training dataset target label
TARGET_LABEL = 'y'
...
def _create_objectives_config(
    ...
    skew_config = ModelMonitoringObjectiveConfig.TrainingPredictionSkewDetectionConfig(
        skew_thresholds=_get_thresholds(all_features, default_threshold, custom_skew_thresholds)
    )

    drift_config = ModelMonitoringObjectiveConfig.PredictionDriftDetectionConfig(
        drift_thresholds=_get_thresholds(all_features, default_threshold, custom_drift_thresholds)
    )

    training_dataset = ModelMonitoringObjectiveConfig.TrainingDataset(target_field=TARGET_LABEL, data_format='csv')
